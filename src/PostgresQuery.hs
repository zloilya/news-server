...
queryNews post@Postgres {..} =
  queryNewsLimitOffset post defLimit (Offset 0) True
...
queryNewsLimit post@Postgres {..} limit =
  queryNewsLimitOffset post limit (Offset 0) True

queryNewsLimitOffset :: Postgres -> Limit -> Offset -> Bool -> IO [News]
...
queryUnpublishNews post@Postgres {..} =
  queryNewsLimitOffset post defLimit (Offset 0) False

queryUnpublishNewsOffset :: Postgres -> Offset -> IO [News]
queryUnpublishNewsOffset post@Postgres {..} offset =
  queryNewsLimitOffset post defLimit offset False

queryUnpublishNewsLimit :: Postgres -> Limit -> IO [News]
queryUnpublishNewsLimit post@Postgres {..} limit =
  queryNewsLimitOffset post limit (Offset 0) False

queryUnpublishNewsLimitOffset :: Postgres -> Limit -> Offset -> UserId -> IO [News]
queryUnpublishNewsLimitOffset Postgres {..} limit0 offset user_id = do
  let limit = min limit0 defLimit
  print "hello2"
  let select =
        "SELECT * FROM "
          ...
          <> " LIMIT ? OFFSET ?"
  print $ select
  let myquery conn = query @_ @NewsRow conn select (False, user_id, limit, offset)
  rows <- bracket (connectPostgreSQL connString) close myquery
  print "hello2"
  mapM action rows
  where
